...
class MyOsf(ZipFile):
    ...
    def print_files_md5_dic(self):
        ...
        _logline("目标productfile.osf路径: {}".format(self.filename))
        for i in sorted(dic):
            _logline("{},{}".format(i, dic[i]))
...
def compare_2_specil_files(file_left, file_right, nbyte=65535):
    ...
    _logline("**************** start ****************")
    _logline("左：{}".format(file_left))
    _logline("右：{}".format(file_right))
    if md5lr[0] == md5lr[1]:
        checksuccess = "是"
    else:
        checksuccess = "否"
    _logline("文件名,文件1MD5,文件2MD5,是否一致")
    _logline("{},{},{},{}".format(filename, md5lr[0], md5lr[1], checksuccess))
    _logline("**************** end ****************\n")
        ...
        if 'backup' not in dirpath and '串口下载-特殊版' not in dirpath:  # 不处理历史和特殊版本
            for fn in filenames:
                if fn.lower() == "productfile.osf":
                    ...
                    l.append(fp)
    _logline("目标productfile.osf路径: {}".format(topdir))
    for p in l:
        _logline(p)
    return l
...
def compare_special_file_to_osf(file_left, file_right):
    ...
    if md5_src == md5_tgt:
        ...
        checksuccess = "否"
    _logline("文件名,文件1MD5,文件2MD5,是否一致")
    _logline("{},{},{},{}".format(filename, md5_src, md5_tgt, checksuccess))
    _logline("**************** end ****************\n")
...
def get_path_pairs(mark_strs, dir_src, dir_tgt):
    ...
    _logline("源镜像个数{}".format(len(path_pairs)))
    # _logline(json.dumps(path_pairs))
...
def compare_all_files_in_osfs(leftosf, rightosfs):
    ...
    for osf in rightosfs:
        ...
        _logline("\n**************** start ****************")
        _logline("左：{}".format(leftosf))
        _logline("右：{}".format(osf))
        l2 = sorted(dic2.keys())
        if len(l1) != len(l2):
            _logline("镜像包内文件列表不一致：{}:\n{}\n{}:\n{}".format(leftosf, l1, osf, l2))
        else:
            _logline("文件名,文件1MD5,文件2MD5,是否一致")
                ...
                if True:
                    if k == "os.ents":
                        if k in dic2.keys():
                            ...
                            _logline("{},{},{},{}".format(k, dic1[k], dic2[k], comparesuccess))
                        else:
                            if "os.dnts" in dic2.keys():
                                comparesuccess = "否,右边是dnts"
                                _logline("{},{},{},{}".format(k, dic1[k], dic2["os.dnts"], comparesuccess))
                            else:
                                _logline("os文件不存在，请重点检查")
                    elif k == "os.dnts":
                        if k in dic2.keys():
                            if dic1[k] == dic2[k]:
                            ...
                            if "os.ents" in dic2.keys():
                                comparesuccess = "否,右边是ents"
                                _logline("{},{},{},{}".format(k, dic1[k], dic2["os.ents"], comparesuccess))
                            else:
                                _logline("os文件不存在，请重点检查")
                    else:
                        if dic1[k] == dic2[k]:
