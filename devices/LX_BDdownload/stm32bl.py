...
def download(port='COM1', file_name='', baud=115200):

    try:
        stm32bl = Stm32bl(port=port, baudrate=baud)
        ...
        stm32bl.mass_erase()
        stm32bl.write_file(address=0x08000000, file_name=file_name)

        stm32bl.cmd_readout_protect()
    except SerialException:
        return "串口打开失败\n"
    except ConnectingException:
        return "无连接到 boot-loader\n"
...
if __name__ == "__main__":
    main()
    # from ctypes import *
    # from middle_board_process import MiddleBoard
    # zhongban = MiddleBoard('DIGITAL MODULE VER1', 'ANALOG MODULE VER1', 'COM7')
    # # cc = Stm32bl('COM7')
    # # # aa.write_digital_bfm(6031, 1, [1, 0])
    # # # time.sleep(1)
    # # zhongban.handle_gongzhuang.write_digital_bfm(6031, 1, [0, 0])
    #
    # # time.sleep(2)
    #
    # stm32bl = Stm32bl('COM5')
    # stm32bl.cmd_readout_unprotect()
    # # stm32bl._connect(5)
    # time.sleep(1)
    # #
    ...
    # #
    # stm32bl.mass_erase()
    # # stm32bl.write_file(address=0x08000000, file_name='G:\近期项目\工装\stm32bl\采集板.bin')
    # #
    # # stm32bl.cmd_readout_protect()
    # #
    # # # zhongban.handle_gongzhuang.write_digital_bfm(6031, 1, [1, 0])
    # # time.sleep(4)
    # # # stm32bl._connect(5)
    # #
    # stm32bl.cmd_go(0x08000000)
    # # zhongban.collect_communication.open_serial(zhongban.serial_port)
    # # aa = zhongban.collect_communication.read_cold_tem()
