...
keyvalue = "key=f1cd9351930d4e589922edbcf3b09a7c"  # headers参数，特定值，研发提供（猜测跟公司绑定）第三方接口key
...
class VnetHttpApi(object):
    ...
    def do_boxes(self):
        		...
        		"list": [
        			{
        				...
        				"boxList": [
        					...
        					},
        					{
        						...
        						"isAlarm": 0,
        						"remark": null,
        						"machineCode": "V04001190626333a2d12a53972e",
        						"boxName": "333 972e",
        						"state": "0",
        						"devModel": "V-BOX S-NG",
        						"boxId": "1425"
        ...
        self.boxlist = r["result"]["list"]
        return r

    def do_realgroups(self):
        params = data_realgroups.copy()
        params["boxId"] = self.boxlist[0]["boxList"][0]["boxId"]
        r = self.post(api_realgroups, params, self.sid)
        self.realgroups = r["result"]["list"]
        return r

    def do_realcfgs(self):
        params = data_realcfgs.copy()
        params["boxId"] = self.boxlist[0]["boxList"][0]["boxId"]
        params["groupId"] = self.realgroups[0]["groupId"]
        r = self.post(api_realcfgs, params, self.sid)
        self.realcfgs = r["result"]["cfgList"]
        return r

    def do_realdata(self):
        """实时监控点数据列表"""
        params = data_realdata.copy()
        params["boxId"] = self.boxlist[0]["boxList"][0]["boxId"]
        params["groupId"] = self.realgroups[0]["groupId"]
        r = self.post(api_realdata, params, self.sid)
        self.realdata = r["result"]["list"]
        return r

    def do_updrealdata(self):
        """修改实时监控点数据"""
        params = data_updrealdata.copy()
        params["monitorId"] = self.realdata[0]["monitorId"]
        params["value"] = str(1)
        r = self.post(api_updrealdata, params, self.sid)
        return r

    def do_monitors(self):
        """获取历史监控点名称列表"""
        r = self.post(api_monitors, {}, self.sid)
        self.monitors = r["result"]["list"]
        return r

    def do_historydata(self):
        """获取指定监控点历史数据"""
        params = data_historydata.copy()
        params["monitorId"] = self.monitors[0]["monitorId"]
        params.pop("monitorBeginTime")
        params.pop("monitorEndTime")
        r = self.post(api_historydata, params, self.sid)
        return r

    def do_alarmdata(self):
        """获取报警数据"""
        params = data_alarmdata.copy()
        params["boxId"] = self.boxlist[0]["boxList"][0]["boxId"]
        params.pop("monitorBeginTime")
        params.pop("monitorEndTime")
        # params.pop("state")
        params["state"] = 1
        params.pop("alarmType")
        params.pop("alarmLevel")
        r = self.post(api_alarmdata, params, self.sid)
        return r
